...
export const useCrud = ({ name, initForm = {}, doCreate, doDelete, doUpdate, refresh }) => {
  ...
  async function handleSave(action) {
    ...
    await validation()
    delete modalForm.value.isTrusted
    delete modalForm.value.currentTarget
    delete modalForm.value._vts
    const actions = {
      add: {
        api: () => doCreate(modalForm.value),
        cb: (data) => $message.success(data.msg || '新增成功'),
      },
      edit: {
  ...
  /** 删除 */
  function handleDelete(id, name = '', confirmOptions) {
    if (!id && id !== 0) return
